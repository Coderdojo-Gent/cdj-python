...
while not game_exit: # not false is true
    for event in pygame.event.get():
        print(event)
        if event.type == pygame.QUIT: # event type "256" is pressing on the cross to close the window
            game_exit = True
        if event.type == pygame.KEYDOWN:
    ...
    if coordX >= screen_width - 10 or coordX < 0 or coordY >= screen_height - 10 or coordY < 0:
        game_over = True
    ...
    while game_over:
        bericht_op_scherm("GAME OVER! Restart the game by pressing 'R' or leave by pressing 'Q'", rood)
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT: # event type "256" is pressing on the cross to close the window
                game_exit = True
